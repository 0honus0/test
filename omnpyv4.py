...
i=0
max_test=3
maxnumber=3   #如果第一次运行，创建个数为maxnumber+1个。如果第二次运行。创建个数为maxnumber+2
begin=0
        ...
        while lis < maxnumber+1:
            try:
                ...
                if lis==0 and not cunzai:
                    ...
                    print(command)
                    stdin, stdout, stderr = client.exec_command(command)
                    command='./nym-mixnode_linux_x86_64 init --id nym'+str(lis)+' --host $(curl ifconfig.me) --port 1789 --incentives-address VJL5tRr66vFVw7tkUpKKheYFGa8oLQawSeRPYtiEJdZyxxE1E6hZA4AU6KcokS8Hcixtq1gAeyKfMBRy'
                    print(command)
                    stdin, stdout, stderr = client.exec_command(command)
                    sleep(8)
                    ...
                    if flag==0:
                        ...
                        if find is False:
                            ...
                            print('copy成功')
                        command='./nym-mixnode_linux_x86_64 init --id nym'+str(lis)+' --host $(curl ifconfig.me) --port '+str(int(port)+int(lis))+' --incentives-address VJL5tRr66vFVw7tkUpKKheYFGa8oLQawSeRPYtiEJdZyxxE1E6hZA4AU6KcokS8Hcixtq1gAeyKfMBRy'
                        stdin, stdout, stderr = client.exec_command(command)
                        sleep(10)
                        command='ls /root/.nym/mixnodes'
                        stdin, stdout, stderr = client.exec_command(command)
                        ...
                        if find is False:
                            ...
                            print('copy成功')
                        command='./nym-mixnode_linux_x86_64 init --id nym'+str(lis+1)+' --host $(curl ifconfig.me) --port '+str(int(port)+int(lis+1))+' --incentives-address VJL5tRr66vFVw7tkUpKKheYFGa8oLQawSeRPYtiEJdZyxxE1E6hZA4AU6KcokS8Hcixtq1gAeyKfMBRy'
                        print(command)
                        stdin, stdout, stderr = client.exec_command(command)
                        sleep(8)
                        command='ls /root/.nym/mixnodes'
